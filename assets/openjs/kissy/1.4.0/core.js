...
KISSY.add(function (S) {
    ...
    function init(frameGroup) {
        ...
        var nodeFuncs = ('index getDOMNodes getDOMNode end equals add item slice scrollTop scrollLeft height width' +
            ...
            'data hasData removeData one').split(' ');

        /**
         * EventObject 的回调属性获取
         */
        var EventObject = KISSY.Event.DomEvent.Object;//兼容kissy1.1.6 和 kissy1.3.0
        ...
        return function (param) {
            ...
            SafeNodeList.prototype.offset = function () {
                if(this.inner[0].FERAL_TWIN___){
                    this.inner[0] = this.inner[0].FERAL_TWIN___
                }
                ...
                DOM: {
                    ...
                    css: frameGroup.markFunction(function (s, name, value) {
                        ...
                        S.each(el,function(v,k){
                            var result = cajaAFTB.checkCss(v, name, value);
                            if (true === result) {
                                DOM.css(v, name, value);
                            }else{
                                S.log(result);
                            }
                        })
